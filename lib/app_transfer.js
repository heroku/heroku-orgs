  ...
  */
  constructor (opts) {
    ...
    if (this.personalAppTransfer) {
      ...
      this.transferMsg = `Initiating transfer of ${cli.color.app(this.appName)} to ${cli.color.magenta(this.recipient)}`;
      this.path = `/account/app-transfers`;
      this.method = 'POST';
    } else {
      this.body = { owner: this.recipient };
      this.transferMsg = `Transferring ${cli.color.app(this.appName)} to ${cli.color.magenta(this.recipient)}`;
